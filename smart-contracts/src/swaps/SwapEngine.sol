...
contract SwapEngine is Ownable {
    ...
    // --------------------------------------------------
    // Authorized LoanManager
    // --------------------------------------------------

    /// @notice LoanManager authorized to close swaps when
    ///         a loan is repaid or liquidated.
    ...
    {
        if (
            swapFactoryAddress == address(0) ||
            settlementEngineAddress == address(0) ||
            lendingPoolAddress == address(0) ||
            dvpEngineAddress == address(0) ||
            swapNFTAddress == address(0) ||
            positionRegistryAddress == address(0) ||
    ...
    modifier onlyLoanManager() {
        if (msg.sender != loanManager) {
            revert UnauthorizedLoanManager();
        }
    ...
    /// @notice Configure the LoanManager authorized to close
    ///         swaps associated with loans.
    ...
    /// @notice Borrower converts a floating-rate loan into
    ///         a hedged position.
    function openSwap(
        uint256 loanTokenId,
    ...
    ///         or liquidated.
    ///
    /// @dev Only the configured LoanManager can call this.
    ...
    function _closeSwap(
        ...
        internal
    {
        swapFactory.markMatured(
            swapId
        );

        swapFactory.closeSwap(
            swapId
        );
